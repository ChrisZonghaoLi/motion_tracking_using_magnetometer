...
int main()
{
 int nlines = 0; // # of columns
 char buf[120], buf2[200];
 double *D, *S, *C, *S11, *S12, *S12t, *S22, *S22_1, *S22a, *S22b, *SS, *E, *d, *U, *SSS, *RAW_unbiased, *RAW_unbiased_T, *Inter_Mat, *Data_cal;
 ...
 double x, y, z; // raw data in LSB
 double a, b, c; // sensor data in mGauss
 double gain = 0.667;

 FILE *fp;
 FILE *fp2;
 // Import Sensor Data Readings
 fp = fopen("C:\\Users\\zongh\\Google Drive\\ENGR 499 Capstone Project\\Project\\Capstone Project Team Folder\\Data_&_SouceCode\\Gym_with_Iron\\Gym_with_Iron.txt", "r");
 fp2 = fopen("C:\\Users\\zongh\\Google Drive\\ENGR 499 Capstone Project\\Project\\Capstone Project Team Folder\\Data_&_SouceCode\\Gym_with_Iron\\Gym_with_Iron.txt", "r");
 ...
 D = (double*)malloc(10 * nlines * sizeof(double)); // Matrix D
 for( i = 0; i < nlines; i++)
 {
   fgets(buf, 100, fp);
   // Convert Sensor data to raw data
...
/*
 C[0] = -1.0;   C[1] = 1.0;   C[2] = 1.0;   C[3] = 0.0;     C[4] = 0.0;     C[5] = 0.0;
 C[6] = 1.0;   C[7] = -1.0;   C[8] = 1.0;   C[9] = 0.0;     C[10] = 0.0;    C[11] = 0.0;
 C[12] = 1.0;  C[13] = 1.0;  C[14] = -1.0;  C[15] = 0.0;    C[16] = 0.0;    C[17] = 0.0;
 C[18] = 0.0;  C[19] = 0.0;  C[20] = 0.0;  C[21] = -4.0;  C[22] = 0.0;    C[23] = 0.0;
 C[24] = 0.0;  C[25] = 0.0;  C[26] = 0.0;  C[27] = 0.0;    C[28] = -4.0;  C[29] = 0.0;
 C[30] = 0.0;  C[31] = 0.0;  C[32] = 0.0;  C[33] = 0.0;    C[34] = 0.0;    C[35] = -4.0;
*/
...
//  printf("%lf\r\n", B[i]);
 ...
 //hm = 0.569; // Define the norm
 printf("\nEnter the Raw Value Total Field: (Kelowna is 367.5)\n");
 scanf("%lf", &hm);

 printf("\nThe Correction Matrix A_1:\n");
 ...
 RAW_unbiased = (double*)malloc(nlines * 3 * sizeof(double)); // Matrix RAW

 for( i = 0; i < nlines; i++)
 {
   fgets(buf2, 200, fp2);
      // Convert Sensor data to raw data
   sscanf(buf2, "%lf\t%lf\t%lf", &a, &b, &c);
   x = gain * a;
   y = gain * b;
   z = gain * c;

   RAW_unbiased[i] = x - B[0];
   RAW_unbiased[nlines + i] = y - B[1];
   RAW_unbiased[nlines * 2 + i] = z - B[2];
 }
/*
 printf ("%lf\t",RAW_unbiased[0]);
 printf ("%lf\t",RAW_unbiased[nlines+0]);
 printf ("%lf\t",RAW_unbiased[nlines*2+0]);
*/
 Data_cal = (double*)malloc(nlines * 3 * sizeof(double)); // Matrix of calibrated data
 //Multiply_Matrices(Data_cal,RAW_unbiased,nlines,3,A_1,3);

 for( i = 0; i < nlines; i++)
 {
   Data_cal[i] = RAW_unbiased[i]*A_1[0] + RAW_unbiased[nlines + i]*A_1[1] + RAW_unbiased[nlines * 2 + i]*A_1[2];
   Data_cal[nlines + i] = RAW_unbiased[i]*A_1[3] + RAW_unbiased[nlines + i]*A_1[4] + RAW_unbiased[nlines * 2 + i]*A_1[5];
   Data_cal[nlines * 2 + i] = RAW_unbiased[i]*A_1[6] + RAW_unbiased[nlines + i]*A_1[7] + RAW_unbiased[nlines * 2 + i]*A_1[8];
 }
/*
 printf ("%lf\t",Data_cal[0]);
 printf ("%lf\t",Data_cal[nlines+0]);
 printf ("%lf\t",Data_cal[nlines*2+0]);
*/
 // Output the calibrated data to a txt file

 FILE *file = fopen("C:\\Users\\zongh\\Google Drive\\ENGR 499 Capstone Project\\Project\\Capstone Project Team Folder\\Data_&_SouceCode\\test.txt", "w");

 for (i=0;i<nlines;i++)
 {
     fprintf (file, "%lf\t",Data_cal[i]);
     fprintf (file, "%lf\t",Data_cal[nlines+i]);
     fprintf (file, "%lf",Data_cal[nlines*2+i]);
     fprintf (file, "\n");
 }
...
// 3. Choleski_LU_Decomposition and Choleski_LU_Inverse -----------------------------------------------------------------------
...
// 4. Lower_Triangular_Solve, Lower_Triangular_Inverse ------------------------------------------------------------------------
